...
module.exports = {
  ...
  'dash.health.months': 'か月分の必要支出をカバー',
  'dash.health.monthsMeta': '目標 {target} · {amount} を指定済み',
  ...
  'dash.health.why.intro': '家計の5つの要素から算出した100点満点の指標です。各要素は全体の一定割合を占め、満点にどれだけ近いかに応じて配点が入ります。ファイルから判断できない要素は0点ではなく対象外になります。',
  'dash.health.why.bands': '{strong} 以上は良好 · {steady}〜{strongLess} は安定 · {steady} 未満は要注意',
  ...
  'dash.health.why.fullReserves': '必要支出 {target} か月分を確保すると満点です。',
  'dash.health.why.fullSaving': '収入の {pct}% を貯蓄すると満点です。',
  'dash.health.why.fullDebt': '利息がゼロで返済もなければ満点です。',
  'dash.health.why.fullSpending': '固定費が収入の {fixed}% 未満、生活費が {living}% 未満、支出が予算内なら満点です。',
  'dash.health.why.fullWealth': '純資産が年収の {times} 倍で満点です — 保有するすべてから負債を引いた額です。',
  'dash.health.why.fixTrim': '生活費を月 {amount} 減らせば収入の {pct}% を下回ります。',
  'dash.health.why.fixBuild': '純資産があと {amount} 増えれば年収の {times} 倍に達します。',
  'dash.health.why.points': '{max} 点中 {points} 点',
  'dash.health.why.dragLabel': '最も失点しているのは{name}で、{points} 点です。',
  'dash.health.why.fixFund': '指定口座に {amount} を追加すれば {target} か月に達し、その分の点が戻ります。',
  'dash.health.why.fixMonthly': '毎月あと {amount} を貯蓄に回せば {pct}% に達し、その分の点が戻ります。',
  'dash.health.why.fixInterest': '毎月 {amount} が何も生まないまま出ています。減らした分はそのまま点数として戻ります。',
  'dash.health.why.allFull': '測定できるすべての要素が満点です。妨げになっているものはありません。',
