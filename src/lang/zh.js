...
module.exports = {
  ...
  'dash.health.months': '个月的必要支出已覆盖',
  'dash.health.monthsMeta': '目标 {target} · 已指定 {amount}',
  ...
  'dash.health.why.intro': '由财务状况的五个部分组成的百分制数字。每个部分占总分的一定比例，你按照距离满分的远近获得相应分数。文件无法回答的部分会被排除，而不是记零分。',
  'dash.health.why.bands': '{strong} 及以上为良好 · {steady}–{strongLess} 为平稳 · 低于 {steady} 需要关注',
  ...
  'dash.health.why.fullReserves': '预留 {target} 个月的必要支出即为满分。',
  'dash.health.why.fullSaving': '储蓄达到收入的 {pct}% 即为满分。',
  'dash.health.why.fullDebt': '没有利息支出且没有分期需要偿还即为满分。',
  'dash.health.why.fullSpending': '固定支出低于收入的 {fixed}%、生活成本低于 {living}%、且支出不超预算即为满分。',
  'dash.health.why.fullWealth': '净资产达到年收入的 {times} 倍即为满分 — 即你拥有的一切减去所有负债。',
  'dash.health.why.fixTrim': '每月减少 {amount} 的生活成本即可降到收入的 {pct}% 以下。',
  'dash.health.why.fixBuild': '净资产再增加 {amount} 即可达到年收入的 {times} 倍。',
  'dash.health.why.points': '{max} 分中的 {points} 分',
  'dash.health.why.dragLabel': '失分最多的是{name}，{points} 分。',
  'dash.health.why.fixFund': '向指定账户再存入 {amount} 就能达到 {target} 个月，并把这些分数拿回来。',
  'dash.health.why.fixMonthly': '每月多存 {amount} 就能达到 {pct}%，并把这些分数拿回来。',
  'dash.health.why.fixInterest': '这是每月 {amount} 却什么也换不到——每还掉一分都会变成分数回来。',
  'dash.health.why.allFull': '所有可衡量的部分都是满分。这里没有拖累你的地方。',
