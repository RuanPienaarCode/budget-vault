...
(async () => {
  ...
  eq(parseOwners('Ruan, Christine'), ['Ruan', 'Christine'], 'a comma list parses');
  eq(parseOwners('  Ruan ,  Christine  '), ['Ruan', 'Christine'], 'whitespace around the names is trimmed');
  eq(parseOwners('[Ruan, Christine]'), ['Ruan', 'Christine'],
  ...
  eq(parseOwners('Ruan, ruan, Christine'), ['Ruan', 'Christine'],
  ...
  eq(parseOwners('Ruan, , ,'), ['Ruan'], 'stray commas do not declare anonymous people');

  eq(ownerKey('  Christine '), 'christine', 'the comparison key is trimmed and folded');
  ...
  eq(S.settings.owners, ['Ruan', 'Christine'], 'the loader parses the settings line onto S');
  eq(byName('His Cheque').owner, 'Ruan', 'an account carries its owner');
  eq(byName('Her Card').owner, 'christine',
  ...
  {
    const fresh = {
      name: 'New Pot', type: 'savings', institution: '', owner: 'Christine',
    ...
    ok(/^owner: "?Christine"?$/m.test(written),
     ...
     the aliases line this app does not model. */
  {
    ...
    a.owner = 'Ruan';                       // put it back for the bands below
    await ctx.saveAccount(a, ctx.ACCOUNT_FM_KEYS);
  }
  ...
  /* ------------- 5. an undeclared owner keeps its account --------------- */
  {
    ...
    eq(opts.slice(0, 3), ['Ruan', 'Christine', JOINT],
    ...
    eq(opts.filter(v => v.toLowerCase() === 'christine'), ['Christine'],
      'a person is offered once, not once per spelling found in the files');

    eq(ownerLabel('christine', declared), 'Christine',
  ...
  /* --------------- 6. the split adds up to the same total --------------- */
  {
    ...
    eq(get('christine').net, 8000 - 3000,
      'both spellings of one person land in one bucket — this is the whole reason for ownerKey');
    eq(get('christine').count, 2, 'and both accounts are counted there');
    ...
    eq(rows.map(r => r.key).slice(0, 3), ['ruan', 'christine', JOINT],
  ...
  /* --------------- 7. the page renders, and can be filtered ------------- */
  {
    ...
    ok(textOf(summary).includes('Christine'), 'and names the people in it');

    const chips = ctx.$('#acctOwners');
    const chipEls = descend(chips).filter(n => n._cls && n._cls.has('acct-seg'));
    // Everyone + Ruan + Christine + Joint + Ouma + unassigned
    ...
    const christineChip = chipEls.find(n => (n.textContent || '').includes('Christine'));
    ok(christineChip, 'there is a Christine chip to click');
    christineChip._fire('click');
    eq(S.acctView.owner, 'christine', 'clicking a chip sets the owner filter');
    ...
    ok(bankChip && /2$/.test((bankChip.textContent || '').trim()),
      `the Bank count is scoped to Christine (cheque + card), got "${bankChip && bankChip.textContent}"`);

    christineChip._fire('click');
    eq(S.acctView.owner, 'christine',
