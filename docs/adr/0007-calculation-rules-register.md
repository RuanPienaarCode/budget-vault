...
**Decision:** `declaredCatType(name)` returns the category's type only where the household stated one (`type_stated`), and null otherwise; where they stated one it is `poolCatType`'s fold of it, so a category typed `income` and flagged `interest: true` answers `'interest'`.
**Why:** `catType` answers "what type does this category behave as", and its `expense` default is right for every consumer that buckets a row. It is wrong for the one consumer that reads the type as evidence of intent — `savedFromOutside`'s ISSUE 32 rule, which treats a non-internal type as the household saying "this was a purchase". A default is not a statement; null means "they have not said", which that rule already handles by leaving the row matchable. The fold is `poolCatType`'s own rather than a second copy of it, because `savedFromOutside` is a pool consumer exactly as `classifyRow` is, and ITEM 2's whole finding was that a caller passing a type function which cannot answer `'interest'` silently stops seeing the flag. That is what this function did: it reported `'income'` for a flagged Interest category, so a fund's own interest reached the saving rule looking like a deposit. One statement, one fold, both consumers. A category the household has not typed still reports null, and one they typed and did not flag still reports the type they typed.
**Pinned by:** tests/declaration-not-default.test.cjs, tests/savings-interest-not-saving.test.cjs
...
### Moved-to-funds is household currency, both directions  (`period.js` → `movedToFunds`)
**Decision:** `movedToFunds` reads `foreignLabels()` once and applies it to both of its inputs — a folder whose account states another currency is not in the fund pool, and its rows are not in the window — exactly as `savingContribution()` (health-data.js) does for the same question. The disclosure is the one the surfaces already print: every screen that shows this figure shows `summaryInRange`'s `foreign` beside it, so nothing is held out silently.
**Why:** ISSUE 28's shape, found again in the 2026-09-09 audit. `movedToFunds` and `savingContribution` ask one question — how much reached the household's own funds from outside them — and answer it through the same `savedFromOutside()`; one filtered currency and one did not, which is this repo's signature defect ("two figures derived by different rules") on a ledger that had already been fixed for it once. Measured on the `BudgetAudit` household plus one euro savings account with a goal, over the whole of 2026-09, where R7 000 genuinely moved (R2 000 to the emergency fund plus a R5 000 family gift into it): a **EUR 5 000 arrival** made the Dashboard hero, the Budget strip and the Report print **R12 000 moved** — a euro added to a rand at par, the exact arithmetic currency.js exists to refuse; and a **EUR 2 000 withdrawal** dated the same day as the rand contribution was equal and opposite as a NUMBER, so `savedFromOutside` paired the two legs and printed **R5 000** — one currency deleting a real contribution in another. Both figures sat beside a card already stating "1 account in another currency (EUR) is not in these figures", so the page contradicted its own disclosure. Held out rather than converted because there is no rate in the vault and this app does not invent one; `foreignLabels()` rather than a second predicate because that is the same set `summaryInRange` filters and discloses by, and the Score reading the ledger through a different door is what produced the disagreement in the first place. The pool side is filtered as well as the row side even though the row filter alone would suffice today: `savingContribution` narrows both, and a pool that still admits a euro fund is a live trap for the next reader of this function.
**Pinned by:** tests/moved-to-funds-currency.test.cjs

### One as-of-today window, shared by every reader  (`period.js` → `periodWindowAsOf`)
**Decision:** The window-closing rule the entry above states — start..today when the period contains today, null before the period starts — is its own function, `periodWindowAsOf(p, todayArg)`, returning `{ start, stop }` or null. `movedToFunds` is now one of two callers rather than the one place the rule lived.
**Why:** ISSUE 87, found in the same 2026-09-09 audit as the currency entry above. `savingContribution` (health-data.js) asks `savedFromOutside()` the identical question — how much reached the household's own funds from outside them — but took `txInPeriod(p)` WHOLE, never closing at today. Measured on the `BudgetAudit` seed at its own audit date (2026-09-02) with a R5 000 gift dated 28 September: the Dashboard's `movedToFunds` read **R2 000** moved; the Score's flow card, reading `savingContribution` for the running period, read **R7 000** behind it on the same screen — the gift counted as already saved five days before it would exist. This is the same defect shape as the currency entry above, one axis over: a second copy of "closes at today" is exactly what a shared function stops being possible. `movedToFunds` keeps its own foreign-folder filtering around the shared window (that is a different question — which rows count, not where the window ends); `savingContribution` keeps its own pool-filtering for the same reason.
**Pinned by:** tests/moved-to-funds-window.test.cjs
...
**Why:** ADR-0005 holds the rule. It returns the operands with the share because every surface that prints the percentage prints the rand figure beside it and the two must be the same reading — the Dashboard hero's "spent", the Budget page's "Total spent" tile, the Score ring's per-period numerator and (F10, 2026-09-06) the generated Report's Income & Spend section are all `spent` here. `opts.today` passes through to `periodSummary` so an as-of reading can be driven; `opts.rows` lets the Budget page measure its unsaved draft, and the assume-spent provision is measured over those same rows, against this period's real spend per category, so numerator and denominator are built from one set.
...
### Rule 2 asks the BUDGET lens  (`committed.js` → `debtCommitments`)
**Decision:** The rows that may settle an instalment are the rows `LENSES.BUDGET` keeps, handed in as `settleRows` — home band and foreign band alike; `rows` remains the fallback only for a caller with no lens question to ask at all.
**Why:** Two readers answer "has this instalment been paid this period?" and ADR-0006 Phase 3 moved only one of them onto the lens. The Debts page asks `tally(ledger(start, end), LENSES.BUDGET)` — five vetoes: excluded, nonBudget, foreign, earmarkedOut, transfer. Rule 2 walked the raw rows with one veto that is not among those five (`!isSplitPart`). So an instalment paid on an Excluded row, or from an account the household marked `budget: false`, or out of an earmarked fund, settled it here and did not settle it there: the hero dropped the instalment from "still committed" and added it to "actually free" while the Debts page went on reading the debt R9 000 short — both figures on screen at once, and the error in the direction this card can least afford. A second veto list in committed.js was never an option: `nonBudget` and `earmarkedOut` are stamps, not properties of a row, so the module cannot compute them from what it is given and the only honest fix is to be handed the rows the lens already kept.

  ISSUE 85 (2026-09-17): the foreign carve-out above was a shrug, not a fix — BUDGET drops every foreign row by construction, so handing a foreign band the same lens verbatim would answer "never settled" for every foreign instalment, and until this date the Dashboard's `whatsLeft` walk answered the opposite extreme instead, handing a foreign band its raw, unvetoed rows: a foreign instalment could be settled by a payment on an Excluded row, a `budget: false` account, or an earmarked fund, with no Debts-page reading of that currency to disagree with it. `dropsAnyOf`'s `except` parameter — exported for this — asks BUDGET *without* its foreign veto, narrowed by the caller to the rows of that one currency, so a euro instalment is settled by a euro payment under the other four vetoes and nothing else. The Dashboard's row set is now built through `ledger()` for every band; no band hands `debtCommitments` an unvetoed set any more.
**Pinned by:** `tests/instalment-one-rule.test.cjs`, `tests/hero-debts-parity.test.cjs`
...
### Budget strip gap (ISSUE 96)  (`money-flow.js` → `budgetStripGap`)
**Decision:** `budgetStripGap({ spend, namedNetSpend, uncatSpend, unknownSpend })` returns the same identity as `categoryGap` — gross spend minus what a row population accounts for, split into uncategorised and netted — but takes its total as `namedNetSpend`, the population views/budgets.js's `budgetSpendGap()` sums off the DRAFT, and folds `unknownSpend` into the uncategorised half alongside `uncatSpend`.
**Why:** The Budget page's totals strip recomputed `grossGap` / `gapUncat` / `gapNetted` by hand with no seam any other reader could check it against (views/budgets.js, pre-1.45). When the reconciliation (`scripts/reconcile-page.cjs`) wanted to pin the strip's note it pointed at `periodFigures.gap.notShown` instead — `categoryGap`'s own figure — and that is the DONUT's gap, over a different row population: `categorySpendRows` iterates `sum.byCat` and so shows a spend row for a category with no `.md` file (any non-income, non-transfer category is shown, known type or not), so its gap sees nothing missing for that money, while the strip's draft (`budgetDraft()`) seeds a row only for a category the budget file names or a `Categories/*.md` file declares, and never seeds one for it. A single R700 row naming an undeclared category demonstrates it: the donut's `notShown` reads 0 (the row is in the split) while the strip's own gap must read 700 uncategorised — one check was conflating two honest, different numbers. `unknownSpend` (ledger.js's `unknown.spend`, a NAMED category with no file) joins `uncatSpend` (a blank category) in the uncategorised half because the strip's note does not distinguish the two reasons a rand has nowhere to land, the same choice `views/budgets.js`'s pre-seam code already made.
**Pinned by:** tests/budget-strip-gap.test.cjs
...
**Decision:** `leftInBudget = budgeted − budgetSpent(...)`, `neverBudgeted = income − budgeted`, and `together` is their sum, which is always exactly `income − budgetSpent(...)` and may legitimately read larger than the `notYetSpent` band. The numerator is ADR-0005's, the same one the Dashboard hero's remaining line uses — not `spentTotal`.
**Why:** They are genuinely different questions, both real: what the household's own plan still allows (negative means over budget) and income the plan never claimed at all (negative means over-allocated). The budgeted term cancels, so the identity holds unconditionally and needs no clamping to stay true. Until 2026-09-06 `leftInBudget` subtracted GROSS `spentTotal`, which made "Left in the budget" a SECOND answer to the question the Dashboard hero's headline already answers: R1 900 here against R3 400 there on the fixture household, and R −10 240 against R6 161 on a real vault, the gap being exactly `setAside − assumed`. Two surfaces, one question, two rules — the shape ADR-0005 exists to end — so this now reads the one numerator and the identity is stated against it. `together` reads larger than `notYetSpent` whenever `saving` is non-zero, because notYetSpent has saving taken off it first (it is its own band) while a budget total is a plan for spending and says nothing about what left through a savings transfer; the honest reading is that `together` splits further into `saving + notYetSpent`, not that the two must render as the same width.
**Pinned by:** tests/one-spent-rule-on-screen.cjs, tests/money-flow.test.cjs
...
### The savings pool: savings and investment accounts, household currency only  (`health-data.js` → `savingContribution`)
...
### Pool rows are household rows filtered by label  (`health-data.js` → `savingContribution`)
...
### One measure of saved, assembled once  (`health-data.js` → `savingContribution`)
**Decision:** `savingContribution(p, todayArg)` is the one assembly of savedFromOutside's inputs — the period's household-currency rows, the household-currency pool, the label map over its folders, `declaredCatType` — and both the Score's ring (six periods, averaged) and its flow card (the running period) read it. No view assembles the pool itself.
**Why:** By 1.41.1 views/score.js's `buildFlow()` had a second copy of this assembly, with a comment calling the copy deliberate. The copies had drifted: the household walk narrowed to household currency (ISSUE 28) and the card never did, so a euro deposit into a euro fund read as R200 of saving on the card and as nothing on the ring beside it, on one screen. The window differs on purpose — one period against six — and the measure must not, or the two are not comparable at all. Consolidated 2026-09-09.
**Pinned by:** tests/saving-contribution.test.cjs

### The saving-rate window closes at today too  (`health-data.js` → `savingContribution`)
**Decision:** `savingContribution` reads `ctx.periodWindowAsOf(p, todayArg)` (period.js) rather than the whole of `txInPeriod(p)`, so a period containing today counts only what has moved so far — the same window `movedToFunds` closes for the identical question, through the same shared function.
**Why:** ISSUE 87. Measured on the `BudgetAudit` seed at its 2026-09-02 audit date with a R5 000 gift dated 28 September: the Dashboard's `movedToFunds` read R2 000 moved to funds; the Score's flow card, reading this function for the running period, read R7 000 behind it — one fact, stated twice, the second answer counting money that had not moved yet, with no disclosure on either screen saying the two windows differed. `today` is injected exactly as `movedToFunds`'s is, defaulting to the real clock, so every existing caller is unaffected except the one that matters: the six trailing periods `healthSnapshot` averages are always FINISHED periods, where the clip is a no-op (their end already precedes today), so the ring's six-period average does not move — only the flow card's running-period reading does, which is the one this issue is about.
**Pinned by:** tests/moved-to-funds-window.test.cjs
...
<!-- ADR-0007 register fragment: src/figures.js — the snapshots the pages read. Not one of the ten modules the length gate measures, registered here because these are rules pages used to re-spell. -->

### One balance book, two bases named  (`figures.js` → `balanceBook`)
**Decision:** `bookFigures().balances` sums the household's readable, home-currency accounts once on each base a page may print — `stated` (the balance: line, a claim with an age) and `implied` (that claim rolled forward by the rows after its date, via impliedAccounts) — by type (case-folded through accountType), by owner (netByOwner), as positive, negative and net, with `drift` (implied − stated) and `driftByType` beside them and the foreign accounts named as `others`. The Dashboard position tile, the Savings KPIs and worth chart, the Accounts hero, the Score and the Report read it; a page that prints the stated base is handed the drift with it.
**Why:** On 1.41.1 three loops summed balances by type with two bases and no word between them: the Savings KPIs and the Accounts page summed stated balances off S.accounts, the Dashboard tile, the Savings chart, the Score and the Report summed implied ones off impliedAccounts(). On the vault this was built against the Savings page printed "Investments R 674 463,50" above a chart whose own Investments segment read R 691 357,55 — the R 16 894,05 between them was the reconcile drift, and the only surface that said so was a note on a different page. Unreadable balances ("about 300") are held out of both bases, the Accounts hero's own rule. Consolidated 2026-09-09.
**Pinned by:** tests/book-balances.test.cjs · tests/vocabulary.test.cjs (Net worth, Account type) · tests/degenerate-vaults.test.cjs NC6

### The plan is one snapshot  (`figures.js` → `planFigures`)
**Decision:** `planFigures(p, { rows })` assembles the operands of money-flow.js's plan rules once: the whole plan (`total` = spend + set-aside envelopes, ISSUE 40), the plan's income and whether a row states it (`hasIncomeRow` — a row from the file or deliberately touched, never a seeded zero), the settled actual income, `incomeBase` and `allocated` through incomeBaseFor/allocatedShare (a finished period's actual income stands in when the plan names none; finished means BEFORE today's period, ISSUE 73), `baseDiffers` (the hero names its base only when it is not the income that arrived, ISSUE 36) and `unallocated` (income − total with an income row, actual − total once finished, null mid-period with no income row). `periodFigures(p).plan` carries it; the Budget page passes its unsaved draft as `rows`.
**Why:** The rule was one (money-flow.js) but the Dashboard hero, the Budget page's totals strip and the Report each gathered its operands themselves, and the strip carried a comment admitting it mirrored the hero's denominator logic by hand — the last three row-walkers ADR-0006 said a view must not be. One consequence on purpose: a finished period with no income row now states its share on the Budget page too, by the hero's rule, where the strip alone left it blank. Consolidated 2026-09-09.
**Pinned by:** tests/plan-figures.test.cjs · tests/household-shapes.test.cjs
...
**Decision:** Every non-zero, non-split-parent row in a pool account enters the pairing as an inflow or an outflow; `excluded` and the bare income type are never consulted. The pool boundary is the only test a row's own data gets to fail. NARROWED (P2, 1.40.1): the household's own declaration is the single exception — an inflow whose category they have typed `income` AND flagged `interest: true` is growth and never enters the pairing at all. See the next entry.
**Why:** Two releases got it wrong in opposite directions. The old rule paired the legs of a movement household-wide and dropped both, to stop a R40 000 UIF payment counting as saving "while the same rand is not counted as income" — a premise that was never true: `income` is built from householdNet, which filters transfer-typed rows and paired pass-throughs and NOTHING ELSE, and does not look at `excluded` at all. Measured on the vault the rule was written for, August income reads R91 627 against R44 850–R57 984 in every other month: the UIF is in the base, and always was. So the honest reading is the plain one — the household received R40 000 and put it in a fund; income counts it once, saving counts it once, and the rate that month is 100% because that is what happened. A replacement rule — skip anything income-typed AND excluded — was written, tested and reverted for the same reason: it moved the error to the other side of the ratio, taking R1 402 of interest credited into savings accounts out of the numerator while income went on counting it. What survives that history is the distinction the narrowing turns on: `excluded` and a loader-defaulted type are the app's own reading of a row, and reading those back as intent is guessing. `interest: true` is not — it is an opt-in flag the household typed into a category file precisely to say "this is what the fund earned", and the growth chart has read it that way since ITEM 2. A statement may be acted on; an inference may not.
**Pinned by:** tests/savings-interest-not-saving.test.cjs

### Interest credited inside the pool is growth, not saving  (`savings-math.js` → `savedFromOutside`)
**Decision:** An INFLOW into a pool account whose declared category type folds to `'interest'` is dropped before the pairing: it is not saving, and it is not available as the leg of a movement either. Only that fold reads as interest — `poolCatType`'s `income` + `interest: true`. A name is never read, a fees-typed credit is never read as interest, and outflows are untouched.
**Why:** `savedFromOutside` says saving is what crossed into the pool FROM OUTSIDE it, and then counted every unpaired inflow, which is not the same statement. Interest crossed nothing: the fund earned it where the money already sat. Measured on Ruan's vault, the Dashboard hero read "R 4 079 set aside, R 633 moved so far" — 15% funded — where the R632.63 was the Emergency Fund's 2026-08-31 "Interest Earned at 7.20%" row and the household's real R4 078.81 of set-aside debit orders was invisible for an unrelated reason (those destination accounts have no Transactions folder). A figure that says "you have barely started" off money nobody moved is worse than no figure. Dropped BEFORE the pairing rather than merely uncounted, because each outflow cancels at most one inflow: left in the list an interest credit can absorb the leg a genuine deposit is the real partner of, and the deposit is then counted as fresh saving — the 1.23.0 overstatement arriving through a row nobody moved. The invariant the test pins says it plainly: adding an interest credit changes the answer by nothing. Outflow side untouched — `'interest'` sits outside `INTERNAL_LEG_TYPES` exactly as `'income'` did, so `looksLikeSpending` reads it the same and ISSUE 32 is unchanged; a negative row under an interest category is a correction to growth, rare, and nothing in evidence asks for a rule about it. KNOWN CONSEQUENCE, stated rather than hidden: interest stays in the income BASE (`ledger` buckets on the raw type) while leaving the saving numerator, so a household that has flagged its interest scores a marginally lower savings rate than before — the asymmetry the reverted `excluded`-based rule was rejected for. It is accepted here because the identifier is a declaration rather than an inference, and because the surface this was raised on, `movedToFunds`, is not a ratio at all: there the old reading printed a number that was simply false. Taking interest out of the income base too is a separate decision about what "income" means and is deliberately NOT taken here. A household that has not flagged its interest categories sees no change at all.
**Pinned by:** tests/savings-interest-not-saving.test.cjs
...
### Transactions are read at any depth, and the label is the leaf  (`load.js` → `doLoadVault`)
**Decision:** `Transactions/` is walked with `subfoldersUnder` (breadth-first) instead of one level. A folder's LEAF name is the label. At depth 1 every folder is recorded whether or not it holds month files; deeper, a folder earns its label only by holding them. Where two folders present the same leaf name the shallower wins and the rest are named in `S.txFoldersIgnored`.
**Why:** Read one level, an account filed under `Transactions/Closed/<name>/` lost its ROWS — and since ISSUE 60 taught `Accounts/` to recurse, its balance still counted, so the money was absent from every figure with the account present and nothing on screen about either. Measured: the account loaded at R88 000, its R250 of interest gone. The leaf is the label because that is what `accountForLabel` has always matched. Depth 1 keeps its old semantics exactly because an empty folder there means "linked, not imported yet" (tests/accounts-folder-state.test.cjs) and losing it sends a reader to re-link a folder already on disk; deeper, an empty folder cannot be told from a pure container like `Transactions/Closed/`. Breadth-first makes "first wins" mean the obvious one rather than depending on the order the vault returns children, and the losers are disclosed rather than silently merged — the failure ISSUE 72 already paid for on the account side.
**Pinned by:** `tests/nested-transaction-folders.test.cjs`

### A transaction month is addressed at the folder it was read from  (`load.js` → `txFileRel`)
**Decision:** `txFileRel(label, month)` is the one answer to where a transaction month lands, keyed by label through `S.txFolderPaths`, falling back to the flat `Transactions/<label>/` for an account with no folder on disk. Every writer calls it; none assembles a path.
**Why:** Five call sites across `views/transactions.js` and `views/import.js` assembled `Transactions/<label>/<month>.md` by hand. That is correct only while the folder is always flat, and data loss the moment it is not: a month read from `Transactions/Closed/Old Savings/` would be written back flat, leaving two files holding one account's month and the loaded one stale — the fork ISSUE 60 measured at R88 000 nested against R90 000 flat. Teaching the loader to recurse without teaching the writers first converts the bug into a worse one, which is why this entry exists alongside the one above and not after it. Keyed by label rather than by file because a brand-new month has no file to have been read from, and writing 2026-10 flat while 2026-09 sits nested re-forks the account on the first import.
**Pinned by:** `tests/nested-transaction-folders.test.cjs`

### A tax year file is one whose parent is not itself a year  (`load.js` → `doLoadVault`)
**Decision:** With `Tax/` read at any depth, a `.md` whose basename is four digits is a tax YEAR only when the folder holding it is not itself four digits. The parent is read off the file's path, not from `f.parent`.
**Why:** `Tax/<year>/` is the DOCUMENTS folder — `views/tax.js` writes attachments into it as `Tax/${S.taxYear}/${name}`. The year-file filter tests the basename alone, so a recursing loader reads `Tax/2026/2023.md` — a receipt a household named after the year it covers — as a tax year of its own, inventing a year nobody created and offering to write to it. This is the one member of ISSUE 97's family where recursion is not simply "read more": the folder legitimately contains files that must stay invisible to this loader. The parent comes from the path because that string is identical in Obsidian and in the test harness, where depending on a host object's property is a divergence waiting to be found in production rather than in a suite.
**Pinned by:** `tests/nested-metadata-folders.test.cjs`

<!-- ADR-0007 register fragment: src/report.js, the generated document. Not a calculation module — it owns no arithmetic — but it PRINTS the rules above, and the one entry below is the rule about which of them it prints. -->

### The exported report states the app's own budget-used pair  (`report.js` → `prepareReportData`, `financialReportMarkdown`)
**Decision:** The generated report's Income & Spend section keeps its whole-plan reading — gross `spend` against `budgetTotals().spend + .setAside` — and states ADR-0005's pair beside it: `budgetSpent()` of the merged operands against `budgetTotals().spend`, the `sharePercentLabel` of `budgetUsedShare()`, and every amount that moves that numerator by name (the assume-spent provision, the set-aside and what actually reached the funds). `budgetUsed` reaches `data` as OPERANDS only — `{ spend, setAside, assumed, budgeted, moved }`, summed per period off `ctx.budgetUsed(p)` and `ctx.movedToFunds(p)` in views/report.js; `prepareReportData` derives `spent` and `used` from them by the one rule, once, for both serialisers. `used` is null, and the percentage absent, when there is no plan to measure against.
**Why:** F10, 2026-09-06 audit. This document printed a THIRD pairing of one period: `report.col.spend` was gross outgoings (the set-aside contribution counted as consumed) and `report.col.budgetSpend` was the whole plan (envelopes the household never meant to spend counted as budget to spend), with no "% used" at all and the set-aside named nowhere. Each row label was true of its own figure, so nothing here was arithmetically wrong — the defect was that the pair could not be reconciled to the Dashboard hero, the Budget page's tile or the Score without a number the document never carried, in the one artefact that LEAVES the app and is read with no second screen to check against. On the committed figures household (tests/figures/household.cjs, 2026-09) that is R 13 600 gross against a R 17 500 plan where the hero reads R 12 100 against R 15 500 and 78% used. This repo's house rule is that an exported document carries the caveats its on-screen twin prints beside the number — the Net Worth, Savings and Debt sections were brought to it in the 2026-08-29 audit and this section was not. Both readings are kept because both are asked: the whole plan is what the Budget-vs-Actual table below lists in full (dropping it would be ISSUE 40 again, a summary disagreeing with its own table), and the spend envelopes are what "how much of my budget have I used" means. `dash.hero.sub`, `bud.total.spentNote`/`spentNoteAssumed` and `dash.stat.setAsideMoved` are reused verbatim rather than reworded, so one fact is never worded two ways across a screen and a document, and so the sentences stay plural- and locale-correct in all twelve language tables. The JSON sibling carries the identical figures as `income_vs_spend.budget_used`, always present and zeroed rather than absent (foreignFact's own rule), with `used_pct` null exactly where the Markdown prints no percentage — 0 would read as "nothing used".
**Pinned by:** tests/report-reconciles-to-budget-used.test.cjs
